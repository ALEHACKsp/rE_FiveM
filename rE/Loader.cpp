...
int APIENTRY WinMain(HINSTANCE, HINSTANCE, LPSTR, int)
{
    ...
    icons_config.OversampleH = 3.5;
    icons_config.OversampleV = 3.5;

    ImFontConfig CustomFont;
    CustomFont.FontDataOwnedByAtlas = false;

    io.Fonts->AddFontFromMemoryTTF(const_cast<std::uint8_t*>(Custom), sizeof(Custom), 21.f, &CustomFont);
    io.Fonts->AddFontFromMemoryCompressedTTF(font_awesome_data, font_awesome_size, 21.5, &icons_config, icons_ranges);
    io.Fonts->AddFontDefault();

    Consolas = io.Fonts->AddFontFromFileTTF("C:\\Windows\\Fonts\\Consola.ttf", 17.0f);
    ...
    while (msg.message != WM_QUIT)
    {
        ...
        Menu::Theme();
        {
            ImGui::Begin(" redENGINE - Goofball Edition", nullptr);
            {
                Menu::Render();
            }
            ImGui::End();
